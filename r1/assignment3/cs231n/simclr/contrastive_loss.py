...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        sim_k = torch.exp(sim(z_k, z_k_N)/tau)
        sum_sim1 = 0
        sum_sim2 = 0
        for m in range(2*N):
            sum_sim1 += torch.exp(sim(z_k, out[m])/tau)if m != k else 0
            sum_sim2 += torch.exp(sim(z_k_N,out[m])/tau)if m != k+N else 0

        total_loss += -torch.log(sim_k/sum_sim1)-torch.log(sim_k/sum_sim2)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = None
    numerator = torch.exp(torch.cat([sim_positive_pairs(out_left, out_right),sim_positive_pairs(out_right, out_left)],dim=0)/tau)

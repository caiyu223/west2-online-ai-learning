...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        # this is what the autograder is expecting. For reference, our solution is #S
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = query.view(N, S, H, self.head_dim).transpose(1, 2)
        key = key.view(N, T, H, self.head_dim).transpose(1, 2)
        value = value.view(N, T, H, self.head_dim).transpose(1, 2) # (N, H, T, E/H)
        scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(self.head_dim)# (N, H, S, T)
        if attn_mask is not None:
            scores = scores.masked_fill(attn_mask.unsqueeze(0).unsqueeze(0)==0, float("-inf"))# (N, H, S, T)
        attn_weights = F.softmax(scores,dim=-1)
        attn_weights = self.attn_drop(attn_weights)
        output1 = torch.matmul(attn_weights, value)# (N, H, S, E/H)
        output = output1.transpose(1, 2).contiguous().view(N, S, E)
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        patch=x.reshape(N,C,H//patch_size,patch_size,W//patch_size,patch_size)
        patch=patch.permute(0,2,4,1,3,5).reshape(N,-1,patch_size*patch_size*C)
        out=self.proj(patch)

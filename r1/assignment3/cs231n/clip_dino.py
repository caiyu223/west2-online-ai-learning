...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    pred_ids = similarity.argmax(dim=0).to('cpu').numpy()
    pred_classes = [class_texts[id] for id in pred_ids]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        text_tokens = clip.tokenize([query]).to(self.device).reshape(1, -1)
        text_features = self.clip_model.encode_text(text_tokens)
        similarity = get_similarity_no_loop(text_features, self.image_features).reshape(-1)

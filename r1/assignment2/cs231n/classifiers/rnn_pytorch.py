...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        word_embedding = word_embedding_forward(captions_in, W_embed)
        if self.cell_type == "rnn":
            out = rnn_forward(word_embedding, h0, Wx, Wh, b)

        out = temporal_affine_forward(out, W_vocab, b_vocab)
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            if i==0:
                start=torch.tensor([self._start]*N).long()
                word = word_embedding_forward(start, W_embed)
            else:
                word = word_embedding_forward(captions[:,i-1], W_embed)
            h = rnn_step_forward(word, prev_h, Wx, Wh, b)
